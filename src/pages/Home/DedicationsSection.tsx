  ...
  return (
    <section className="relative py-24 sm:py-32 px-5 sm:px-8">
      ...
      <motion.div
        ...
        className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 max-w-4xl mx-auto px-4"
      >
        {quotes.map((quote, index) => (
          <motion.div
            key={index}
            variants={fadeInUp}
            className="glass rounded-2xl p-5 sm:p-6 md:p-8 relative overflow-hidden group hover:bg-white/8 transition-all duration-500"
            whileHover={{ y: -2 }}
          >
            ...
            <Heart
              ...
              strokeWidth={0}
            />
            <p className="text-white/50 text-xs sm:text-sm md:text-base font-light leading-relaxed italic">
              "{quote}"
            </p>
      ...
      >
        <div className="inline-flex items-center gap-2 sm:gap-3 glass rounded-full px-5 sm:px-6 py-2.5 sm:py-3">
          <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-lila-400" fill="currentColor" strokeWidth={0} />
          <span className="text-white/40 text-xs sm:text-sm font-light">Y todo lo que no puedo decir con palabras</span>
          <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-lila-400" fill="currentColor" strokeWidth={0} />
        </div>
      </motion.div>
    </section>
